...
STAGE = "Staging" 
LOG_FORMAT = "%(levelname)s:%(asctime)s:%(module)s:%(message)s" # Define a structured log format
...
def load_model():
    ...
    try:
        ...
        if not versions:
            error_msg = f"Model '{MODEL_NAME}' in stage '{STAGE}' not found in registry."
        ...
        logger.info(f"Successfully loaded model '{MODEL_NAME}' version {version} from {STAGE} stage.")
        
    except Exception as e:
        logger.exception(f"FATAL ERROR during model loading: {e}")
        # Re-raise or exit to indicate a critical startup failure
        raise RuntimeError("API failed to initialize due to critical model loading error.") from e
...
async def predict_risk(customer_data: CustomerFeatures):
    ...
    logger.info(f"Received prediction request for customer data: {customer_data.dict()}")
    
    if model is None:
        logger.error("Prediction request failed: Model is not loaded (or initialization failed).")
    ...
    try:
        ...
        logger.info(f"Prediction successful. Probability: {risk_prob:.4f}, Label: {risk_label}")
