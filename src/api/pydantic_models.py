...
class CustomerFeatures(BaseModel):
    ...
    R_Min_Days: float = Field(..., description="Recency (Minimum Days since last transaction).")
    F_Count: float = Field(..., description="Frequency (Total transaction count).")
    M_Debit_Total: float = Field(..., description="Monetary (Total debit/spending amount).")
    M_Debit_Mean: float = Field(..., description="Monetary (Mean debit/spending amount).")
    M_Debit_Std: float = Field(..., description="Monetary (Standard Deviation of debit/spending amount).")
...
class PredictionResponse(BaseModel):
    ...
    risk_probability: float = Field(..., description="The probability (0 to 1) that the customer is high risk (Class 1).")
    risk_label: str = Field(..., description="The classification label (High Risk or Low Risk).")