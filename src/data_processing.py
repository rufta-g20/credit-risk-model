...
class RFMAggregator(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        cluster_analysis = rfm_df.groupby('Risk_Cluster').agg(
            Avg_Recency=('R_Min_Days', 'mean'),
            Avg_Frequency=('F_Count', 'mean'),
            Avg_Monetary=('M_Debit_Total', 'mean')
        ).sort_values(by=['Avg_Recency', 'Avg_Frequency'], ascending=[False, True]) # Sort by highest Recency (least engaged)
        ...
        rfm_df['is_high_risk'] = np.where(rfm_df['Risk_Cluster'] == high_risk_cluster_id, 1, 0) # Final column name is 'is_high_risk'
...
class WOETransformer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        X_feats = X.drop(columns=['is_high_risk'], errors='ignore') # Drop target before transformation
...
if __name__ == '__main__':
    ...
    print("Task 4: Proxy Target Engineering is complete. Data is ready for model training.")